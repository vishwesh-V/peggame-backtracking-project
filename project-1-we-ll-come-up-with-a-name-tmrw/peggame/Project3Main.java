...
public class Project3Main {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        while(!userInputCorrect){ //gets file name
                ...
                fileName = tempStr;
                try{
                    board = TriangleReader.readTriangleBoard(fileName);
    ...
    public static void updateState(Collection<Move> possibleMoves){
        
        if(possibleMoves.isEmpty()) {
            int temp_count = 0;
            
            boolean[][] boardVals = board.getPegs();
            for (int i = 0; i < boardVals.length && temp_count <= 2; i++) {
                for (int j = 0; j < boardVals[i].length && temp_count <= 2; j++) {
